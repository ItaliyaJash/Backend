...
const registerUser = asyncHandler(async (req, res) => {
    // get user deatails from fromtend
    // validation - not empty
    // check if user already exists: username, emsil
    // check for images , check for avatar
    // upload them to cloudinary, avatar
    // create user object - create entry in DB
    // remoe password and refersh token field from response
    ...
    if (!avatarLocalPath) {
        throw new ApiError(400, "Avatar file is requirede")
    }
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    const longgedInUser = await User.findById(user._id).select("-password -refreshToken")
    ...
    return res
        ...
        .json(
            new ApiResponse(
                200,
                {
                    user: longgedInUser, accessToken, refreshToken
                },
